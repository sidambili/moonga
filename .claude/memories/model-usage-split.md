...
description: Which LLM providers/models Moonga actually runs in practice
metadata:
  type: project
---

~99% of Moonga agent runs go through **OpenRouter with DeepSeek** (or other open-source models); only ~1% use OpenAI, and only when a user specifically wants GPT-5.5 with high/xhigh thinking effort. GPT-4.x is considered obsolete and is not used.
