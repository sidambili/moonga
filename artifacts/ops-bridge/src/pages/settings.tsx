...
const MODEL_SUGGESTIONS: Record<string, { triage: string[]; plan: string[] }> = {
  openai:    { triage: ["gpt-4o-mini", "gpt-4o"], plan: ["gpt-4o", "gpt-4-turbo"] },
  anthropic: { triage: ["claude-3-haiku-20240307"], plan: ["claude-3-5-sonnet-20241022"] },
  openrouter:{ triage: ["openai/gpt-4o-mini"], plan: ["openai/gpt-4o"] },
...
export default function ModelSettings() {
  ...
  const handleSave = () => {
    updateMutation.mutate({
      data: { provider, triage_model: triageModel, plan_model: planModel, api_key: apiKey || undefined, base_url: baseUrl || undefined }
    }, {
      onSuccess: () => {
  ...
  return (
    <div className="p-4 md:p-6 max-w-2xl mx-auto space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">BYOK — bring your own key and route by task type</p>
      </div>

      <div className="rounded-xl bg-card border border-border/60 p-4 space-y-5">
        {settings && (
          <p className="text-xs text-muted-foreground">Last updated {formatDate(settings.updated_at)}</p>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            { label: "Triage Model", value: triageModel, set: setTriageModel, hints: suggestions.triage, desc: "Fast, cheap — classification and Slack summaries" },
            { label: "Plan Model",   value: planModel,   set: setPlanModel,   hints: suggestions.plan,   desc: "More capable — deep diagnosis and planning" },
          ].map((m) => (
            <div key={m.label} className="space-y-1.5">
              <Label className="text-xs text-muted-foreground flex items-center gap-1.5">
                <Cpu className="w-3 h-3" />{m.label}
              </Label>
              <Input
                value={m.value}
        ...
        <Button onClick={handleSave} disabled={updateMutation.isPending} className="w-full rounded-lg">
      ...
      <div className="rounded-xl bg-card border border-border/60 p-4 space-y-3">
        ...
        <div className="space-y-3">
          {[
            { tag: "Triage", desc: "Initial classification, severity assessment, and Slack summaries. Optimized for speed." },
            { tag: "Plan",   desc: "Deep diagnosis, implementation planning, and incident reports. Optimized for accuracy." },
          ].map((r) => (
